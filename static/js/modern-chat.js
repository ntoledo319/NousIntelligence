...
class NOUSModernChat {
  ...
  displaySearchResults(results) {
    ...
    if (results.length === 0) {
      this.dom.search.results.innerHTML = `<div class="p-3 text-sm text-gray-500">No results</div>`;
    } else {
      this.dom.search.results.innerHTML = results
        .map(
          (result) => `
  ...
  displayNotifications(notifications) {
    ...
    if (notifications.length === 0) {
      this.dom.notifications.list.innerHTML = `<div class="p-3 text-sm text-gray-500">No notifications</div>`;
    } else {
      this.dom.notifications.list.innerHTML = notifications
        .map(
          (notification) => `
